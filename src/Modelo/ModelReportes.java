...
public class ModelReportes {
    ...
    public boolean BuscarAccesos(String par, Connection con){
                ResultSet rs;
        try {
            String query = "SELECT tbAccesos.idAcceso AS tbAccesos_idAcceso, tbAccesos.fecha AS tbAccesos_fecha, tbAccesos.hora AS tbAccesos_hora, tbAccesos.notificacion AS tbAccesos_notificacion, tbTipoAccesos.tipo_acceso AS tbTipoAccesos_tipo_acceso, CONCAT(tbPersonal.nombre_p, ' ', tbPersonal.apellido_p) AS tbPersonal_nombre_p, tbPersonal.Carnet AS tbPersonal_Carnet FROM dbo.tbTipoAccesos tbTipoAccesos INNER JOIN dbo.tbAccesos tbAccesos ON tbTipoAccesos.idTipoAcceso = tbAccesos.idTipoAcceso INNER JOIN dbo.tbPersonal tbPersonal ON tbAccesos.idPersonal = tbPersonal.idPersonal WHERE tbAccesos.fecha = ?";
            ps = con.prepareStatement(query);
            ps.setString(1, par);
            rs = ps.executeQuery();
